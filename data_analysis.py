...
def turn_r_files_to_speadsheet(folder):
    with open("aggregate_data/" + folder + ".csv", "w") as g:
        print(
            "Year,Semester,Department,Code,num_students,Mean,SD,Kurtosis,Skewness,Dip,p(Dip),Shapiro,p(Shapiro),AD,p(AD),p(BIMT),LR-BIMT,logShap,logpShap",
            file=g,
        )
        for fname in os.listdir("r_output/" + folder):
            with open("r_output/" + folder + "/" + fname, "r") as f:
                ...
                print(
                    *content, sep=",", file=g, end=""
                )  # no \n needed at end because python is weird


def analyze_spreadsheet(folder):
    data = np.genfromtxt(
        "aggregate_data/" + folder + ".csv", delimiter=",", skip_header=1
    )
    p_shapiro = data[:, 12]
    ...
    print(
        n,
        "tested\t",
        round((n_shapiro_rejected / n) * 100, 2),
        "% rejected and are not-normal\t",
        round((n_shapiro_fail_to_reject / n) * 100,),
        "% fail to reject",
    )
    ...
    print(
        num_bimod,
        "reject NH (and so are multimodal)\t",
        round((num_bimod / n) * 100, 2),
        "% of total are multimodal",
    )
